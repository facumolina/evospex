...
public final class LinkedTreeMap<K, V> extends AbstractMap<K, V> implements Serializable {
  ...
  /**
   * Create a tree map ordered by {@code comparator}. This map's keys may only be null if
   * {@code comparator} permits.
   *
   * @param comparator
   *          the comparator to order elements with, or {@code null} to use the natural ordering.
   */
  @SuppressWarnings({ "unchecked", "rawtypes" }) // unsafe! if comparator is null, this assumes K is
                                                 // comparable
  ...
  void removeInternal(Node<K, V> node, boolean unlink) {
    ...
    if (left != null && right != null) {
      ...
      /*
       * To remove a node with both left and right subtrees, move an adjacent node from one of those
       * subtrees into this node's place.
       *
       * Removing the adjacent node may change this node's subtrees. This node may no longer have
       * two subtrees once the adjacent node is gone!
       */
  ...
  private void rotateLeft(Node<K, V> root) {
    ...
    // fix heights
    root.height = Math.max(left != null ? left.height : 0, pivotLeft != null ? pivotLeft.height : 0)
        + 1;
